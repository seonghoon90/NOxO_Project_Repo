...
# staging 여부 확인 (exit 1 = staged diff 존재, exit 0 = no staged diff)
is_staged() {
    local file="$1"
    # 파일이 git에 새로 추가된 경우(untracked X, staged new)도 staged로 간주
    if git diff --cached --quiet -- "$file" 2>/dev/null; then
        # no staged diff → staged new file 여부 체크
        if git ls-files --error-unmatch "$file" &>/dev/null; then
            # 이미 tracked → no staged change
            return 1
        else
...
# 새로 추가된(git에 없던) 파일이 staged 인지 확인
is_new_staged() {
    local file="$1"
    git diff --cached --name-only 2>/dev/null | grep -qF "$file"
}

# 파일이 staged 상태인지 통합 판단
file_is_staged() {
    local file="$1"
...
for DIR in "${DIRS[@]}"; do
    ...
    # Case A: CLAUDE.md만 staged → CLAUDE.md → AGENTS.md 복사
    ...
    # Case B: AGENTS.md만 staged → AGENTS.md → CLAUDE.md 복사
    ...
    if [[ $CLAUDE_STAGED -eq 1 && $AGENTS_STAGED -eq 1 ]]; then
        if ! cmp -s "$CLAUDE" "$AGENTS"; then
            ...
            # 둘 다 staged이지만 내용 동일 → OK, 그냥 통과
            echo "[SYNC] $LABEL: 둘 다 staged, 내용 동일 → OK"
        fi
        continue
    fi

    # Case D: 둘 다 unstaged + 내용이 다름 → 충돌
